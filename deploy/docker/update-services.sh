...
"${SCP_BASE[@]}" "$SCRIPT_DIR/ensure-env-secrets.sh" "$HOST:/tmp/ensure-env-secrets.sh"
awk -F'[ =]' 'NF>=3 && $0 !~ /^#/ && $0 !~ /^$/ {print $1, $2, substr($0, index($0, $2) + length($2) + 1)}' \
  "$UPDATE_ENV_FILE" | "${SSH_BASE[@]}" "$HOST" "set -e; cp '$UPDATE_INSTALL_DIR/.env' /tmp/update-services.env; sh /tmp/sync-server-env.sh /tmp/update-services.env /tmp/update-services-compose.yml; sh /tmp/ensure-env-secrets.sh /tmp/update-services.env; docker compose -f /tmp/update-services-compose.yml --env-file /tmp/update-services.env config --quiet; TS=\$(date +%Y%m%d%H%M%S)-\$\$; cp '$UPDATE_INSTALL_DIR/.env' '$UPDATE_INSTALL_DIR/.env.bak-\$TS'; cp '$UPDATE_INSTALL_DIR/docker-compose.yml' '$UPDATE_INSTALL_DIR/docker-compose.yml.bak-template-\$TS'; cp /tmp/update-services.env '$UPDATE_INSTALL_DIR/.env'; cp /tmp/update-services-compose.yml '$UPDATE_INSTALL_DIR/docker-compose.yml'"

#!/usr/bin/env sh
# 内部脚本：将标准输入中的 "SERVICE VAR VALUE" 幂等同步到服务器 .env 和 Compose。
set -eu

if [ "$#" -lt 2 ]; then
  echo "用法: sync-server-env.sh <env_path> <compose_path> [compose_path...]" >&2
  exit 2
fi
...
wire_compose() {
  ...
  if awk -v header="  $service:" -v key="      $variable:" '
    ...
    END { exit found ? 0 : 1 }
  ' "$compose_file"; then
    echo "已接线，跳过: $(basename "$compose_file") $service $variable"
    return
  fi
  ...
  ' "$compose_file"; then
    echo "服务不存在或没有 environment 段: $(basename "$compose_file") $service" >&2
    exit 1
  fi
  ...
  echo "已接线: $(basename "$compose_file") $service $variable"
}
...
while IFS=' ' read -r service variable value; do
  ...
  for compose_file in "$@"; do
    wire_compose "$service" "$variable" "$value" "$compose_file"
  done
done < "$entries_file"

...
- `livekit/livekit.yaml` 会在服务器安装成 `/home/mounts/media/livekit.yaml`。
- `livekit/livekit-ingress.yaml` 会在服务器安装成 `/home/mounts/media/livekit-ingress.yaml`。
- Ingress 的 WHIP 监听固定使用宿主机内部端口 `18081`，避免与现有服务的 `8080` 冲突；首期 RTMP 接入不需要对公网开放该端口。
- `livekit/livekit-egress.yaml` 会在服务器安装成 `/home/mounts/media/livekit-egress.yaml`。
...
- 推荐把 tdt 地图文件打成 `nginx/html/tdt.zip`，安装时会解压到 `/home/mounts/media/nginx/html/tdt`。

...
实时录像基于当前 `VideoSession` 发起 LiveKit Egress，但正式回放不直接使用 live HLS。Egress 先输出 MP4 到录像对象路径 `original/source.mp4`，停录后复用录像模块的 HLS worker 生成 VOD HLS，只有 `READY` 录像可播放。
...
| `POST` | `/api/control/video-sessions/{sessionId}/recordings/{recordingId}/stop` | 停止当前浏览器发起的录像 |
...
7. 停录后录像状态进入 `VERIFYING`，后台生成 VOD HLS；完成后进入 `READY` 并出现在回放列表。
...
|---|---|
| 方法 | `POST` |
| 路径 | `/api/control/video-sessions/{sessionId}/snapshots/file` |
| 调用方 | 前端 -> Control Server |
...
| `remark` | string | 否 | 人工备注，例如 `云台-可见光 手动抓拍` |
| `clientCapturedAt` | datetime | 否 | 前端点击抓拍并截帧的时间 |
| `previewImageHash` | string | 否 | 当前以前端 JPEG blob size 作为轻量标记 |
| `file` | file | 是 | 前端从当前 `<video>` 画面通过 canvas 截取的 JPEG 图片 |

响应类型：`SnapshotResponse`。成功时状态通常直接为 `COMPLETED`，`officialObjectKey` 指向 MinIO 中的正式图片。

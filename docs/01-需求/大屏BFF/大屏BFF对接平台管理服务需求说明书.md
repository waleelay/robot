...
说明：BFF REST 输出 `devices[]` 时不再使用整套 mock 数据兜底；管理端或控制端未提供的标量字段返回 `null`，数组字段返回空数组。但当前 WebSocket 适配对 `test111/SN005/SN006` 保留演示位置，该兼容应在生产移除或配置化。

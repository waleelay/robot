...
说明：当前实现已移除控制会话续租和 active 查询接口，前端以 acquire/takeover 返回的会话作为当前控制权来源。
...
当前 `control-service` 已移除 `POST /api/control/robots/{robotId}/control-sessions/{controlSessionId}/heartbeat`。控制会话由 acquire/takeover 创建，服务端根据 `leaseExpireAt` 清理过期会话；前端释放控制权时调用 release。
...
当前 `control-service` 已移除 `GET /api/control/robots/{robotId}/control-sessions/active`。前端以 acquire/takeover 返回的当前会话作为本地控制权状态来源，必要时重新 acquire。

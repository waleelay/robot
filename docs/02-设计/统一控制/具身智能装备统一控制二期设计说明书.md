...
| 文档属性 | 内容 |
|---|---|
| 文档性质 | 二期目标设计，当前未作为 Java 实现基线 |
| 当前代码基线 | 2026-08-12 |
| 当前接口权威文档 | [控制服务接口文档](../../03-接口与协议/统一控制/控制服务接口文档.md) |

本文描述后续扩展目标。机械臂轨迹、点云、多厂商 driver 插件、协议协商、命令回放与完整审计不在当前 Java 实现内；文中的接口或字段除非同时出现在当前接口权威文档和代码中，否则不得用于联调或验收。
...
二期可以在后端审计、链路追踪、回放系统中增强 `policy`、`traceId`、操作者上下文等治理信息，但默认不把这些字段作为 Python 机器人客户端执行指令的必需参数。
...
每个 action 都应有 schema，用于前端渲染、后端校验和 Python driver 适配。
...
- 默认与本体运动冲突。
- 默认要求设备级或整机强独占。
- Python driver 必须做软限位、碰撞区域和驱动状态校验。
...
以下接口只是二期候选，当前 Java Controller 均未注册：
...
POST /api/control/robots/{robotId}/control-sessions/force-takeover
```

`force-takeover` 仅作为二期高权限强制抢占能力，不参与一期普通模式切换流程。
...
Python 机器人客户端应以 `target/deviceId/action` 为路由键提供 driver 扩展点；具体目录和接口以届时的 Python 实现为准，不保留已删除的 Go 客户端目录约束。
...
二期验收场景见[统一控制二期验收用例](../../04-测试与验收/验收用例/统一控制二期验收用例.md)。

...
| 文档属性 | 内容 |
|---|---|
| 文档性质 | 总体目标设计，包含当前实现与后续治理目标 |
| 当前代码基线 | 2026-08-23 |
| 当前接口权威文档 | [控制服务接口文档](../../03-接口与协议/统一控制/控制服务接口文档.md) |

涉及真实 REST/WebSocket 路径、字段、状态码和已实现范围时，以当前接口权威文档及代码为准；本文中的审计、ACK、限流、断线自动 stop、插件化与高级安全策略属于设计目标，未全部落地。
...
| 文档 | 用途 |
|---|---|
| [具身智能装备统一控制一期设计说明书](具身智能装备统一控制一期设计说明书.md) | 一期实现范围，覆盖设备注册绑定、控制权、本体/云台遥控、捕网器、急停、WebSocket、MQTT |
| [具身智能装备统一控制二期设计说明书](具身智能装备统一控制二期设计说明书.md) | 二期扩展范围，覆盖机械臂、激光雷达、任务接管、driver 插件化、协议版本和审计回放 |
...
- 不同厂商设备：松灵机器人、G1四足机器狗、后续其他厂商机器人与上装。
...
现有实时视频链路已经使用 `Control Server -> MQTT -> Python 机器人客户端` 的模式。远程控制模块建议作为与 `media` 并列的 `control/teleop` 能力扩展，而不是混入实时视频 session。
...
- `robotId` 决定命令发给哪台机器人和哪个 Python 机器人客户端。
...
5. Python 机器人客户端无法稳定按 `deviceId` 路由到不同 driver。
...
说明：当前没有独立控制会话续租和 active 查询接口。takeover 专用于导航模式转人工本体控制，只申请空闲控制权，不强制抢占其他终端。
...
生产请求必须先经 Bigscreen BFF 验证 JWT，由 BFF 覆盖并注入以下用户上下文 Header。Control 当前不独立验签 JWT而是信任这些 Header，因此只允许在受控内网直连；本地调试手工设置 Header 不属于生产鉴权能力。
...
用途：查询当前用户可见的机器人摘要列表，用于左侧/地图/调度列表展示。前端从响应体 `devices` 字段读取机器人列表；历史 `GET /api/control/robots` 已移除。
...
| `VEHICLE_LIGHT` / `light.vehicle.set` | `front`、`rear` | 前后车灯状态，包含平台通用 `mode`、`brightness`。 |

当前实现不生成设备初始 `status`。客户端只能上报从机器人端真实读取或确认的音量、发射器、云台、警示灯和车灯状态；缺失状态由前端显示“同步中”或保留本地临时态，不能作为设备真实状态。平台侧 `deviceId`、动作和控制参数来自管理端登记信息；为保持既有 MQTT 协议不变，已完成端到端联调的设备类型可使用生产兼容动作和参数范围，但该兼容数据不包含运行状态。
...
当前没有独立的控制会话 heartbeat 接口。控制会话由 acquire 或导航人工接管创建，合法本体移动请求自动续期，服务端根据 `leaseExpireAt` 清理过期会话；前端释放控制权时调用 release。
...
当前没有单独查询活跃控制会话的接口。前端以 acquire/takeover 返回的当前会话作为本地控制权状态来源，必要时重新申请；冲突时 acquire 响应中的 `holder` 提供占用方信息。
...
后端消费 `robot/{robotId}/media/client/status` 后，先合并管理端设备 ID、动作和控制参数，再更新机器人注册状态并向前端广播一条 `robot.state`。每条客户端状态上报只产生一条合并事件。
...
| `controlMode` | string | 是 | 控制模式；缺省为 `手动模式` |
...
- Python 机器人客户端必须幂等处理，多次 stop 不应产生副作用。
...
- `ptz.stop` 不能被后端限流丢弃。
- Python 机器人客户端必须停止对应 `deviceId` 的云台运动。
...
- Python 机器人客户端仍应按真实设备能力做本地安全、冷却和设备状态校验。
...
Python 机器人客户端可订阅 `robot/{robotId}/control/#`，topic 只区分设备大类，具体设备按 payload 中的 `target.deviceId/action` 分发。
...
### 7.4 后端与 Python 机器人客户端 MQTT 消息明细
...
| `seq` | number | 是 | 沿用前端终端递增序号，客户端用于日志和观察丢帧/乱序 |
| `issuedAt` | datetime | 是 | 后端下发时间 |

Python 机器人客户端处理要求：

1. `robotId` 不匹配时丢弃并记录日志。
2. 按 `target.deviceType + action` 分发给对应设备处理逻辑。
3. `drive.velocity` 从 `params.linear.x/y/z` 与 `params.angular.roll/yaw` 读取运动量；一期地面机器人忽略预留轴之外的扩展字段。
4. 找不到 driver 或 action 不支持时记录 `UNSUPPORTED`。
5. `operator`、`controlSessionId`、`policy`、`confirmToken` 等字段不应作为客户端执行依赖。

#### 7.4.2 急停命令 `robot/{robotId}/control/safety/estop`

方向：后端 -> Python 机器人客户端。

用途：最高优先级急停。Python 机器人客户端收到后应优先停止本体和高风险运动设备。
...
用途：统一上报客户端在线状态、控制模式、任务状态、摄像头清单和设备运行状态。平台设备 ID 和能力由后端查询管理端后合并；前端实时状态通过合并后的 WebSocket `robot.state` 获取。
...
|---|---|---:|---|
| `robotId` | string | 是 | 机器人 ID |
| `clientId` | string | 是 | Python 机器人客户端实例 ID |
| `status` | string | 是 | 媒体模块已有在线状态字段 |
| `controlMode` | string | 是 | `手动模式` / `导航模式` |
| `controlModeName` | string | 是 | 后端返回的中文名称：`手动模式` / `导航模式` |
| `stateSeq` | number | 是 | 客户端状态递增序号，前端切换模式时携带 |
| `missionStatus` | string | 否 | 任务状态 |
| `navigationStatus` | string | 否 | 导航状态 |
...
后端按管理端控制画像的 `maxLinearX/maxLinearY/maxAngularZ` 分别限幅。`maxLinearY=0` 表示不支持横移；轮式和双足兼容画像默认禁用横移，只有管理端登记非零上限后才开放。`linear.z` 与 `angular.roll` 为无人机预留，一期地面机器人固定下发 `0.0`。
...
```json
{
  ...
  "action": "drive.velocity",
  "params": {
    "linear": {"x": 0.25, "y": 0.1, "z": 0.0},
    "angular": {"roll": 0.0, "yaw": 0.15}
  },
  "issuedAt": "2026-06-02T10:30:00+08:00"
}
```

四足机器狗的横移能力同样由 `maxLinearY` 决定。`gait`、`bodyHeight`、`stabilityMode` 不混入一期 `drive.velocity`；后续如接入，应作为独立能力和 action 定义。
...
机器人侧 Python 客户端在 `python-client/robot_media_client/` 中处理 MQTT 订阅、状态上报和设备模拟；生产设备可按 `target/deviceId/action` 实现独立 driver，不再保留 Go 客户端目录或接口约束。
...
| `RobotMediaStatusSubscriber` | 订阅 `media/client/status`，调用设备控制服务合并管理端登记信息，再刷新机器人状态并推送一条 WebSocket 事件 |
...
- G1四足机器狗绑定双光云台、捕网器、机械臂、语音对讲时，前端自动展示这套组合。
- G2四足机器狗绑定双光云台、探照灯时，前端不展示捕网器、机械臂、激光雷达控制。
...
- MQTT topic 按设备域拆分，Python 机器人客户端订阅 `robot/{robotId}/control/#`，状态统一上报 `robot/{robotId}/media/client/status`。
...
这样设计后，同一套前端和后端接口可以同时覆盖R1轮式机器人、G1四足机器狗、双光云台、捕网器、机械臂、激光雷达以及未来更多厂商设备。平台负责统一治理，机器人侧负责厂商协议适配，二者边界清晰，后续扩展成本最低。

...
说明：当前实现已移除控制会话续租和 active 查询接口。takeover 专用于导航模式转人工本体控制，只申请空闲控制权，不强制抢占其他终端。
...
当前 `control-service` 已移除 `POST /api/control/robots/{robotId}/control-sessions/{controlSessionId}/heartbeat`。控制会话由 acquire 或导航人工接管创建，合法控制请求自动续期，服务端根据 `leaseExpireAt` 清理过期会话；前端释放控制权时调用 release。
...
当前 `control-service` 已移除 `GET /api/control/robots/{robotId}/control-sessions/active`。前端以 acquire/takeover 返回的当前会话作为本地控制权状态来源，必要时重新申请。

...
| Python 机器人客户端 | 按 `sessionId` 管理音频收发，采集麦克风、播放下行音频、按需启动视频发布 |
| LiveKit | 在既有 Room 中转发视频与双向音频 Track |

### 3.1 浏览器生命周期约束

- 普通机器人对讲和现场视频呼叫共享一个浏览器麦克风租约，从 `STARTING` 开始互斥，直到本地媒体释放完成。
- 挂断、拒接、接听超时、页面退出、注销、设备离线及服务端终态统一取消在途连接，并停止底层 `MediaStreamTrack`。
- LiveKit `disconnect()` 属于尽力而为的网络清理，最多等待 3 秒；超时或异常不能阻塞本地状态、DOM 音频元素和通话租约释放。
- 已拒接、超时、结束或失败的 `callId` 在浏览器保留 60 秒终态记录。期间收到乱序或重复的 `accepted` 时只向服务端挂断，不允许重新打开麦克风。
- 结束对讲时，若当前 Room 仍承载视频观看，只释放麦克风和对讲状态；没有视频消费者时才断开 Room。

普通扬声器音量不使用命令响应作为最终事实。拖动和按钮操作只提供即时交互反馈；每次设备状态包都会生成前端接收序号，即使边缘端没有更新时间或连续上报相同值，也会使用最新 `devices.status.volume/muted` 撤销本地乐观值。多合一设备保持独立控制流程，不参与普通扬声器映射。
...
|---|---|---|---|
| `audio.robot.mic` | Python 机器人客户端 | 操作员前端 | 对讲开启后必需 |
| `audio.operator.mic` | 操作员前端 | Python 机器人客户端 | 对讲开启后必需 |
| `video.{channel}.{quality}` | Python 机器人客户端 | 前端 | 用户观看视频时启用 |
...
sequenceDiagram
    ...
    participant GO as Python 机器人客户端
...
后端复用规则依赖 `robotId/deviceId/channel/quality` 一致。Control Server 调用 Media Service 创建/复用会话；当复用到 `intercomAudioOnly=true` 且无视频 `trackSid` 的会话时，会继续调用 `client-start` 并向 Python 机器人客户端下发 `robot/{robotId}/media/video/start`，在同一个 Room 内补充视频 Track。
...
| Topic | 方向 | 说明 |
|---|---|---|
| `robot/{robotId}/media/video/intercom/start` | Control -> Python 机器人客户端 | 在指定视频会话 Room 内启动音频 |
| `robot/{robotId}/media/video/intercom/stop` | Control -> Python 机器人客户端 | 停止指定视频会话内的音频 |
| `robot/{robotId}/media/video/intercom/status` | Python 机器人客户端 -> Control | 上报对讲状态和音频 Track |
...
| 通话挂断且仍在观看 | 前端取消发布并停止本地麦克风 Track，保留当前 LiveKit Room 和视频 Track |
| 通话挂断且无人观看 | 前端取消发布并停止本地麦克风 Track、断开 LiveKit Room；服务端按现有空闲释放策略关闭视频会话 |
| 对讲启动中关闭入口、切换页面、注销，或被叫接听中收到结束事件 | 取消当前启动请求并等待操作收口；LiveKit Room 连接和麦克风启用等待都可被生命周期信号抢占；迟到成功的 Room 再次断开，迟到创建的麦克风轨道立即停止，已取得 `sessionId` 的服务端占用立即回滚 |
| 地图控制弹窗关闭 | 只结束该弹窗实际绑定摄像头的对讲并释放麦克风，再释放该弹窗的视频消费者；不影响同机器人其他摄像头或其他页面消费者 |
| 大屏页面退出 | 结束当前页面仍在进行的对讲并释放麦克风，再停止当前页面实际观看的视频；不停止仅在后台执行的独立录像，也不关闭登录级业务 WebSocket |
| 用户注销或身份刷新 | 结束全部对讲并释放所有本地麦克风 Track，再停止视频、录像并关闭业务 WebSocket，执行登录级全量清理 |
| 对端结束或通话失败 | 不依赖组件是否仍显示，统一释放本地麦克风和远端音频元素；无人观看时同时断开 Room |
| 对讲心跳超时 | Media Service 标记 `video.intercom.interrupted`，Control Server 下发 `intercom/stop`；无人观看时进入 `IDLE_WAIT` |
| 机器人离线、权限移除或 LiveKit 最终断开 | 立即停止本地麦克风 Track、清理远端音频并尽力结束服务端对讲；当前实现不自动恢复对讲，需要用户重新发起通话 |
| 挂断接口超时或失败 | 服务端确认不得阻断本地关闭；浏览器立即停止麦克风、分离音频并释放通话互斥租约，服务端残留占用由对讲心跳超时机制最终收口 |
...
- 通话中的“麦克风静音”只 mute 当前发布 Track，浏览器仍保持采集属于正常行为；“挂断/结束”必须 unpublish 并 stop 本地 Track，不能复用静音语义。
- 同一浏览器全局只允许一条非多合一通话流程；普通机器人对讲和现场视频呼叫共享同一通话租约，用户点击接听即进入 `STARTING` 互斥，直到本地媒体完全释放。多合一设备使用独立的设备会话和媒体流程，不纳入该浏览器对讲租约。
...
| Python 机器人客户端 | 订阅 `robot/{robotId}/media/video/intercom/start|stop`，按 `sessionId` 加入 LiveKit Room，发布 `audio.robot.mic` 并订阅 `audio.operator.mic` |
| 运维 | 沿用 LiveKit 当前端口和媒体转发部署；长时间通话依赖前端心跳与 Control 定时超时扫描 |

机器人客户端音频依赖：

```text
LiveKit Python SDK: 负责加入 Room、Opus 编解码、发布和订阅 Audio Track
...
验收步骤见[实时视频语音对讲验收用例](../../04-测试与验收/验收用例/实时视频语音对讲验收用例.md)。

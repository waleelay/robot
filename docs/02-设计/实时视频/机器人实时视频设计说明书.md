...
1. 机器人客户端上报的 Track SID 是由 `sessionId` 派生的占位值，Media 不会像固定摄像头一样在进入 `STREAMING` 前通过 LiveKit Room API 确认真实 Track。
...
5. Media 当前不订阅 LiveKit Webhook，机器人视频 Track 以客户端 MQTT 状态为主；浏览器端仍应以 LiveKit Track 事件判断实际画面是否就绪。

## 12. 关键配置与验证

| 模块 | 关键配置 |
| --- | --- |
| Media | `LIVEKIT_*`、`TRACK_PUBLISH_TIMEOUT_SECONDS`、`VIEWER_HEARTBEAT_TIMEOUT_SECONDS`、`IDLE_RELEASE_DELAY_SECONDS` |

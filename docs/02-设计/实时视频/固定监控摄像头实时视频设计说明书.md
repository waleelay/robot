...
固定摄像头作为装备列表中的同级对象返回。BFF 可以扩展现有装备列表或全景概览接口，不为固定摄像头单独设计任务维度接口。

```http
GET /api/bigscreen/panorama/equipment
```
...
BFF 只聚合展示字段，不返回 RTSP 地址和认证信息。固定摄像头装备项必须明确标识 `sourceType=FIXED_CAMERA`，前端据此隐藏控制中心和控制器操作。
...
sequenceDiagram
    ...
    FE->>BFF: GET /api/bigscreen/panorama/equipment
    BFF-->>FE: equipment list with fixed cameras

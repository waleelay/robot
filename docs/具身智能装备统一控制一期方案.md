...
`devices[].status` 用于刷新前端有状态控件。一期统一规则：机器人端若提供某个设备的真实状态接口或状态上报，客户端应把结果写入 `devices[].status`，前端必须优先按 `status` 渲染；没有真实状态时才使用本地临时态或展示“同步中”。当前实现默认随客户端/后端设备定义提供：扬声器 `volume=50`、`volumePercent=50`、`muted=false`，发射器 `connected=true`、`safetySwitchEnabled=false`、`tubeCount=6`、`tubes[]=LOADED`，双光云台 `autoRotateEnabled=false`、`panSpeed=0`，左右警示灯 `enabled=false`。车灯 `front/rear` 不写默认值，仅在收到 `light.vehicle.set` 或真实设备状态后上报；前端在状态未知时允许下发车灯命令，但不会把初始化占位写入缓存。

...
`devices[].status` 用于刷新前端有状态控件。当前实现默认随客户端/后端设备定义提供：扬声器 `volume=50`、`volumePercent=50`、`muted=false`，双光云台 `autoRotateEnabled=false`、`panSpeed=0`，左右警示灯 `enabled=false`。车灯 `front/rear` 不写默认值，仅在收到 `light.vehicle.set` 或真实设备状态后上报；前端在状态未知时允许下发车灯命令，但不会把初始化占位写入缓存。

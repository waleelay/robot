...
> 状态：已废弃，仅作为历史设计参考。
>
> 当前实现已统一为通用文件上传、存储与播放方案，录像、抓拍、任务产物、日志、配置和其他附件均使用 `media_file` / `media_file_upload` / `media_video_file` 以及 `/api/media/files`、`/api/control/files` 接口。请以 [通用文件上传、存储与播放方案](./通用文件上传存储与播放设计.md) 为准。旧的 `/api/media/recording-uploads`、`/api/media/recordings`、`/api/control/recordings`、`media.recording.*` 配置和 `recording/` 后端模块已移除。

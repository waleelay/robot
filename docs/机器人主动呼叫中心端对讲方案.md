...
```http
GET /api/control/intercom-calls?status=RINGING
```

用途：

- 前端刷新当前未处理来电。
- WebSocket 短暂断开后恢复来电弹窗。
...
```http
POST /api/control/intercom-calls/{callId}/accept
```
...
3. 校验当前机器人是否已有对讲占用。
4. 将呼叫状态更新为 `ACCEPTED`。
5. 创建或复用 `VideoSession`。
6. 调用现有 Media Service 对讲接口。
7. 向机器人下发 `robot/{robotId}/media/video/intercom/start`。
8. 向前端返回 `IntercomResponse`。
...
```json
{
  ...
  "livekitUrl": "wss://192.168.124.77:4443/livekit",
  "operatorToken": "eyJ...",
  "expiresAt": "2026-07-14T10:45:08+08:00"
}
```
...
```http
POST /api/control/intercom-calls/{callId}/reject
```
...
1. 校验呼叫是否仍为 `RINGING`。
2. 将呼叫状态更新为 `REJECTED`。
3. 向机器人发送 `call/state`，状态为 `rejected`。
4. 向前端广播 `video.intercom.call.rejected`。
...
- 如果该 `sessionId` 关联了机器人主动呼叫记录，则把呼叫状态更新为 `ENDED`。
- 向机器人发送 `call/state`，状态为 `ended`。
...
建议在 Control Service 新增来电记录表，例如 `robot_intercom_call`。

| 字段 | 类型 | 说明 |
|---|---|---|
| `call_id` | varchar(64) | 主键 |
| `robot_id` | varchar(64) | 机器人 ID |
| `device_id` | varchar(64) | 绑定摄像头 ID |
| `channel` | varchar(24) | 视频通道 |
| `quality` | varchar(16) | 清晰度 |
| `reason` | varchar(255) | 呼叫原因 |
| `status` | varchar(24) | 呼叫状态 |
| `session_id` | varchar(64) | 接听后关联的 `VideoSession` |
| `accepted_by` | varchar(64) | 接听用户 |
| `accepted_client_id` | varchar(128) | 接听页面实例 |
| `created_at` | datetime | 呼叫发起时间 |
| `expires_at` | datetime | 呼叫过期时间 |
| `accepted_at` | datetime | 接听时间 |
| `ended_at` | datetime | 结束时间 |
| `last_message` | varchar(512) | 最近状态说明 |

索引建议：

| 索引 | 字段 | 用途 |
|---|---|---|
| `idx_robot_intercom_call_robot_status` | `robot_id,status` | 查询某机器人当前来电或占用 |
| `idx_robot_intercom_call_status_expires` | `status,expires_at` | 定时扫描超时来电 |
| `idx_robot_intercom_call_session` | `session_id` | 通过会话回写呼叫结束状态 |
...
sequenceDiagram
    ...
    FE->>CS: POST /intercom-calls/{callId}/accept
...
| 中心端多人同时接听 | 数据库条件更新保证只有一个成功，其余返回已被接听 |
...
| 前端 WebSocket 断开 | 重连后调用 `GET /api/control/intercom-calls?status=RINGING` 恢复弹窗 |
| 服务重启 | 通过呼叫表恢复未过期 `RINGING`；已过期的更新为 `TIMEOUT` |

## 11. 权限与安全

1. 只有具备对讲权限的用户可以接听。
2. 接听接口需要校验用户所在组织是否有权限访问该机器人。
...
### 13.2 Control Service

- 新增来电 DTO。
- 新增来电状态枚举。
- 新增来电实体和 Repository。
- 新增 MQTT 订阅器，处理 `call/invite`、`call/cancel`。
- 新增 MQTT 发布方法，发送 `call/state`。
- 新增来电 Service，封装接听、拒接、超时、占线逻辑。
- 新增 Controller，提供查询、接听、拒接接口。
- 新增定时任务，扫描超时来电。
- 接听成功后复用现有 `ControlVideoCommandService.startIntercom` 类似逻辑。

### 13.3 Media Service

- 无需新增来电表。
- 无需新增独立音频 Room。
- 如现有接口满足接听后创建对讲，不需要改造。
- 如需要记录 `callId` 和 `sessionId` 的关联，可由 Control Service 维护。
...
| 8 | 机器人取消来电 | 前端弹窗关闭 |
| 9 | 前端 WebSocket 断开后恢复 | 仍能通过查询接口恢复未超时来电 |

## 15. 总结

本方案把“机器人主动呼叫中心端”拆成两层：

```text
呼叫信令层：机器人发起来电，中心端振铃、接听、拒接、超时
媒体通话层：接听后复用现有 VideoSession + LiveKit 对讲
```

这样可以在最小改动现有媒体链路的前提下，实现机器人主动呼叫、中心端手动接听的业务能力。

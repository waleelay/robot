...
固定摄像头 Gateway 在 `rtspUrl` 缺失时，默认从当前有效租约目录按 `sourceId` 解析，所以
重启/切换命令仍可执行；仅 `FIXED_CAMERA_CATALOG_MODE=management` 兼容模式会直接查询
Management。RTSP URL 和 publisher Token 均为敏感载荷；Control 已改为白名单日志摘要，固定
...
| Gateway | `FIXED_CAMERA_GATEWAY_ID`、`FIXED_CAMERA_CATALOG_MODE`；旧 `management` 模式另需 `MANAGEMENT_SERVICE_URL/TOKEN`；RTSP/publisher 配置；`FIXED_CAMERA_HTTP_ADDR` 仅开放内部 `/health`、`/metrics` |

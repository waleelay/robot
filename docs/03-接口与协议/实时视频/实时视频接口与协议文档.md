...
浏览器 -> BFF -> Control -> Media
                           -> EMQX -> Python 机器人客户端 / Fixed Camera Gateway
...
以上均使用 QoS 1。当前没有独立 ACK Topic；是否已发布由 status 流转表示。`catalog/sync`
...
Control 只接受有效期不超过 300 秒、签发时间偏差不超过 30 秒且版本递增的租约，并将所有
...
- 固定摄像头不复用机器人心跳上线恢复；Gateway 由非在线恢复为在线时重启当前仍有观看者的固定视频源，单路 RTSP 由不可用恢复为可用时只重启该源。首次健康快照、重复在线状态和无观看者会话不触发重启；浏览器 Track/Room 异常只恢复 viewer，不重启 Publisher。
...
4. LiveKit Webhook 只用于触发对账，最终状态以 Room API 当前事实为准；Publisher generation 尚未纳入本项，留待后续整改。
...
| 模块 | 配置 |
| --- | --- |
| Media | `LIVEKIT_*`、`LIVEKIT_RECONCILE_DELAY_MS`、`TRACK_PUBLISH_TIMEOUT_SECONDS`、`INTERRUPTED_GRACE_SECONDS`、`IDLE_RELEASE_DELAY_SECONDS`、`VIEWER_HEARTBEAT_TIMEOUT_SECONDS` |

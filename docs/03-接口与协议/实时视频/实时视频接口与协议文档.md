...
固定摄像头 Gateway 在 `rtspUrl` 缺失时会使用 `sourceId` 查询 Management，所以重启/切换命令仍可执行。RTSP URL 和 publisher Token 均为敏感载荷；Control 已改为白名单日志摘要，固定摄像头 Gateway/Publisher 不再记录命令参数、完整载荷或外部进程输出。历史日志治理和部署后采样要求见[生产调用链路风险与整改清单](../../02-设计/生产调用链路风险与整改清单.md)。

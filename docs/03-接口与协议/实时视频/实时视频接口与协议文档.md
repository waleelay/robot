...
当前代码中 Control Server 与 Media Service 已拆成独立 Spring Boot 服务。Control Server 通过 `ControlMediaServiceClient` 以 HTTP 方式调用 Media Service 的 `/internal/media/**`，不直接依赖 Media 的 Service、Repository 或 Controller。
...
```text
media.test111.camera01.visible
media.test111.camera02.visible
media.SN006.camera04.visible
```
...
```text
test111 -> Go Client A -> camera01/camera02/camera03
SN006 -> Go Client B -> camera04/camera05/camera06
```
...
Control API: http://{serverIp}:8082/api/control
Media Internal API: http://{serverIp}:8088/internal/media
LiveKit: ws://{serverIp}:7880
业务 WebSocket: ws://{serverIp}:8090/ws/control 或 ws://{serverIp}:8082/ws/control
```
...
### 5.2 `RobotDeviceResponse`（机器人列表聚合模型）
...
| `type` | string | 是 | 机器人类型，例如 `轮式机器人`、`四足机器狗` |
| `battery` | number | 是 | 机器人电量百分比，整数 `0-100` |
| `status` | string | 是 | `online` / `offline` |
| `controlMode` | string | 是 | 控制模式：`手动模式`、`导航模式` |
...
| `lastHeartbeatAt` | datetime | 可选 | Control Service 最近收到客户端状态或心跳的时间 |
...
| `sourceType` | string | 是 | 媒体源类型：`ROBOT_CAMERA` / `FIXED_CAMERA` |
| `sourceId` | string | 是 | 媒体源 ID；机器人摄像头为 `robotId`，固定摄像头为 `cameraId` |
...
| `participantIdentity` | string | 是 | 发布者身份，例如 `robot:test111:camera01` |
...
Control API 是前端主要直接调用的业务接口，当前由独立 `control-service` 暴露，路径为 `/api/control/**`；涉及媒体会话、Token、文件等能力时，Control Service 通过 HTTP 调用 Media Service 的 `/internal/media/**`。

### 6.1 查询机器人与摄像头列表（当前走 BFF 聚合）

| 项 | 内容 |
|---|---|
| 方法 | `GET` |
| 路径 | `/api/bigscreen/panorama/overview` |
| 调用方 | 前端 -> Bigscreen BFF |

请求参数：无。

响应类型：从响应体 `devices` 字段读取 `RobotDeviceResponse[]`。`GET /api/control/robots` 已移除，BFF 对该路径返回 `410 Gone`。
...
数据来源由 BFF 聚合平台管理设备、实时状态和本地兜底数据；Control Service 仍通过 MQTT 维护机器人在线状态，用于控制画像、WebSocket 推送和上线恢复推流。
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---:|---|
| `robotId` | string | 是 | 要观看的机器人 ID，来自 `/api/bigscreen/panorama/overview.devices` |
...
```text
1. Control Server 将前端请求转换为 Media Service 的 CreateVideoSessionRequest。
2. Media Service 查找同 robotId + deviceId + channel + quality 且状态为 ROOM_READY/STREAMING/IDLE_WAIT 的可复用会话。
3. 如果存在，Media Service 新增或刷新当前 X-Client-Id 对应 viewer，返回新的 viewerToken；Control Server 不重复下发 start。
4. 如果不存在，Media Service 只新建 INIT VideoSession、记录 viewer 并返回 viewerToken；此阶段不创建 publisherToken，也不下发 MQTT。
5. Control Server 看到返回状态为 INIT，继续调用 /internal/media/video-sessions/{sessionId}/client-start。
6. Media Service 在 client-start 阶段创建 LiveKit Room、签发 publisherToken、生成 commandId，并把会话置为 REQUESTING_CLIENT。
7. Control Server 将返回的 VideoStartCommand 发布到 EMQX，再查询一次 Media Service 返回最终 VideoSessionResponse 给前端。
```
...
首次观看时，前端优先使用 start 响应里的 `viewerToken` 和 `livekitUrl`，不额外调用该接口。只有 start 响应缺少 token、页面恢复、LiveKit token 过期、非主动断线重连或前端显式要求 refreshToken 时，才调用该接口重新签发观看 Token。
...
前端调用的对外接口仍然是本节路径 `/api/control/video-sessions/{sessionId}/restart`，不直接调用 Media Service 的内部接口。

响应类型：`VideoSessionResponse`。Control Server 会先向 Media Service 请求 `restart-command`，下发 MQTT start 后再查询一次会话，因此当前响应会带新的 `viewerToken`；如果响应缺少 `viewerToken` 或 `livekitUrl`，前端再调用 `/token` 获取新 Token。

用于非主动停止时的恢复。前端只在当前画面没有 stopped/stopping 且状态为 `STREAMING/INTERRUPTED` 时触发。

后端行为：Control Server 调用 Media Service 的 `/restart-command`。Media Service 重新创建 Room、签发 publisherToken、生成 commandId 并将状态改为 `REQUESTING_CLIENT`；Control Server 负责把返回的 start 命令下发到 EMQX。
...
| Tab | 数据来源 | 展示/操作 |
|---|---|---|
| 手动录像 | `GET /api/control/files?robotId=...&fileType=VIDEO&status=READY&page=0&size=20` 后过滤 `extensionId` 为空 | 左侧展示 READY 视频文件，点击后签发播放 URL 并在右侧 HLS 播放 |
| 巡逻录像 | `GET /api/control/files?robotId=...&fileType=VIDEO&status=READY&page=0&size=20` 后过滤 `extensionId` 非空 | 左侧展示任务/巡逻视频文件，点击后签发播放 URL 并在右侧 HLS 播放 |
...
Media Internal API 是 Media Service 暴露给 Control Server 的内部能力接口，当前路径为 `/internal/media/**`。Control Server 通过 HTTP client 调这些接口，不直接调用 Media 内部类。

| Control Server 调用 | Media Service 内部接口 | 说明 |
|---|---|---|
| `POST /api/control/robots/{robotId}/cameras/{deviceId}/video/start` | `POST /internal/media/video-sessions` | 创建或复用视频会话，返回 viewerToken；新会话此时仍为 `INIT` |
...
| `POST /api/control/video-sessions/{sessionId}/restart` | `POST /internal/media/video-sessions/{sessionId}/restart-command` + `GET /internal/media/video-sessions/{sessionId}` | 生成恢复推流命令并返回带 viewerToken 的会话 |
...
| Control 客户端状态订阅 | Control 本地 `RobotRegistryService.update(...)` | Control 更新机器人在线/设备状态；若从离线变在线，再调用 Media 的 `online-restart-commands` 恢复推流 |
...
| `sessionId` | string | 是 | 视频会话 ID |
| `robotId` | string | 是 | 机器人 ID |
| `sourceType` | string | 否 | 媒体源类型，默认 `ROBOT_CAMERA`；固定摄像头为 `FIXED_CAMERA` |
| `sourceId` | string | 否 | 媒体源 ID；固定摄像头 Gateway 用该字段查询管理端摄像头档案 |
...
### 9.4 固定摄像头 Gateway Topic

固定摄像头不走 `robot/{robotId}/...` topic，也不上报机器人在线心跳。Control Server 按 `sourceType=FIXED_CAMERA` 将命令下发到固定摄像头 Gateway：

| 场景 | Topic | Payload |
|---|---|---|
| start | `gateway/fixed-camera/{gatewayId}/video/start` | 同 9.1，`sourceType=FIXED_CAMERA`，`sourceId=deviceId=cameraId` |
| stop | `gateway/fixed-camera/{gatewayId}/video/stop` | 同 9.2，额外可带 `sourceType/sourceId/deviceId` |
| restart | `gateway/fixed-camera/{gatewayId}/video/restart` | 同 9.1 |
| status | `gateway/fixed-camera/{gatewayId}/video/status` | 同 9.5 |

Gateway 收到 start/restart 后使用 `sourceId` 查询管理端 `/api/v1/management/fixed-cameras`，选择 `mainStreamUrl/subStreamUrl` 并复用 RTSP 探测和 publisher 推流。命令 payload 不携带 RTSP URL、账号或密码。

### 9.5 Go 客户端上报视频状态

| 项 | 内容 |
|---|---|
| Topic | `robot/{robotId}/media/video/status`；固定摄像头为 `gateway/fixed-camera/{gatewayId}/video/status` |
| QoS | 1 |
| 方向 | Go 客户端或固定摄像头 Gateway -> Control Server -> Media Service |
...
ROBOT_ID=SN006
ROBOT_CLIENT_ID=robot-media-client-SN006
```
...
sequenceDiagram
    ...
    ML->>ML: create/reuse VideoSession + create viewerToken
    ML-->>CS: VideoSessionResponse(status=INIT/reusable, livekitUrl, viewerToken)
    alt new session or audio-only session without video track
        CS->>ML: POST /internal/media/video-sessions/{sessionId}/client-start
        ML->>ML: CreateRoom + create publisherToken + commandId
        ML-->>CS: VideoStartCommand
        CS->>MQ: video/start
        CS->>ML: GET /internal/media/video-sessions/{sessionId}
        ML-->>CS: VideoSessionResponse(status=REQUESTING_CLIENT, livekitUrl, viewerToken)
    end
...
| 4 | `Media Service + LiveKit: create/reuse VideoSession + create viewerToken` | Media Service 在创建/复用阶段只维护 VideoSession 和 viewer，并为当前浏览器签发观看 Token；此阶段不生成 publisherToken。 |
| 5 | `Media Service + LiveKit -> Control Server: VideoSessionResponse` | 返回 `sessionId`、`roomName`、`livekitUrl`、`viewerToken`、`status`、`viewerCount` 等会话信息。新会话状态为 `INIT`；复用会话通常为 `ROOM_READY/STREAMING/IDLE_WAIT`。 |
| 6 | `Control Server -> Media Service + LiveKit: POST /internal/media/video-sessions/{sessionId}/client-start` | 仅新建或对讲 audio-only 会话升级视频时调用。Media 创建 LiveKit Room，签发 publisherToken，生成 `commandId` 和 start 指令 payload，并将会话置为 `REQUESTING_CLIENT`。 |
...
| 8 | `Control Server -> Frontend: VideoSessionResponse` | 新会话下发 start 后，Control Server 再查询一次 Media Service 并把包含 `livekitUrl` 与 `viewerToken` 的响应返回前端；复用会话则直接返回第 5 步响应。首次观看直接使用该 Token，不额外请求 `/token`。 |
...
当前代码中 Control Server 与 Media Service 独立部署，Control 通过 HTTP 调用 /internal/media/**；本地默认配置中 Control 指向 `http://localhost:8088`。
```

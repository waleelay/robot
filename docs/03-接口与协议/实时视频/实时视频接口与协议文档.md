...
| 机器人 `streaming` / `track_published` | `STREAMING` |
| 固定摄像头 `streaming` / `track_published` | 先保持 `ROOM_READY`；Media 通过 LiveKit Room API 查到真实视频 Track 后转为 `STREAMING` |
...
固定摄像头 Gateway 的 `streaming` 表示推流进程已经通过快速启动检查，不承诺 LiveKit 已发布真实
Track，也不提供占位 `trackSid`。Media 是固定摄像头 Track 状态的唯一确认方；首次发布阶段进程退出
直接失败，已进入真实 `STREAMING` 后的断流才进入既有中断恢复流程。
...
| 模块 | 配置 |
| --- | --- |
| Media | `LIVEKIT_*`、`TRACK_PUBLISH_TIMEOUT_SECONDS`、`INTERRUPTED_GRACE_SECONDS`、`IDLE_RELEASE_DELAY_SECONDS`、`VIEWER_HEARTBEAT_TIMEOUT_SECONDS` |

...
固定摄像头 Gateway 在 `rtspUrl` 缺失时会使用 `sourceId` 查询 Management，所以重启/切换命令仍可执行。RTSP URL 和 publisher Token 均为敏感载荷；当前 Control 的 MQTT 发布/异常日志仍可能序列化完整 payload，生产前必须改为字段级脱敏，见[生产调用链路风险与整改清单](../../02-设计/生产调用链路风险与整改清单.md)。

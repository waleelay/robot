...
|---|---|---|---|---|
| 客户端在线状态、心跳、摄像头清单和设备状态 | MQTT QoS 1 | `robot/{robotId}/media/client/status` | MQTT 连接成功、重连成功、周期心跳、正常退出、部分状态变化后刷新 | `control-service` 的 `RobotMediaStatusSubscriber.clientStatusListener` -> `RobotRegistryService.update(...)` |
...
| `type` | string | 是 | 机器人类型展示值，如 `轮式机器人`、`四足机器狗`。 |
| `battery` | number | 是 | 电量百分比，建议整数 `0-100`。 |
| `status` | string | 是 | 客户端原始在线状态，常见 `online` / `offline`。 |
| `controlMode` | string | 是 | 控制模式，只允许 `手动模式`、`导航模式`；当前大屏模式以下文边缘状态 topic 的值为准。 |
| `stateSeq` | number | 是 | 客户端状态递增序号；前端切换模式时会携带观测到的状态序号。 |
...
| `devices` | array | 否 | 客户端掌握的本体和上装设备运行态；平台设备能力由控制服务与管理端登记组件合并。 |
...
1. `media/client/status` 是机器人基础状态、视频源清单和设备运行状态的统一入口；平台设备能力由控制服务查询管理端后合并。
2. `stateSeq` 必须单调递增。前端切换控制模式时会用它判断状态是否过旧。
3. 视频 `streaming` 和对讲 `active` 状态应尽量携带 Track SID/Track Name，否则前端可能只能看到状态变化，无法精确绑定媒体 Track。
4. 设备 MQTT 上报的 `timestamp` 使用 ISO-8601 带时区格式；Java REST/WebSocket 输出使用上海时区 `yyyy-MM-dd HH:mm:ss`。
...
7. `devices[].deviceId` 对外以管理端组件 ID 为准；驱动侧 ID 不一致时写入 `status.driverDeviceId`。控制服务依次按平台 ID、驱动侧 ID、唯一设备类型关联状态。
8. 管理端设备查询使用 30 秒缓存；刷新失败时使用最近一次成功数据，避免短时管理端故障改变既有控制协议。

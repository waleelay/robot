...
| `controlMode` | string | 是 | 控制模式，只允许 `MANUAL`、`NAVIGATION`。 |
...
| `control.controlMode` | `controlMode` | 中英文兼容归一为 `MANUAL` 或 `NAVIGATION`；未知值不覆盖已有模式。 |
...
| `controlMode` | string | 是 | 控制模式编码：`MANUAL`、`NAVIGATION`。 |
| `controlModeName` | string | 是 | 后端补充的中文名称：`手动模式`、`导航模式`。 |

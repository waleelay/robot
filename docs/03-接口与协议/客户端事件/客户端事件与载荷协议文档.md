...
同一机器人同时上报两类状态时按字段归属合并：边缘状态负责在线、本体电量、
运动、控制模式、任务和定位；`media/client/status` 负责媒体客户端心跳、摄像头
和上装设备。边缘状态到达后的 30 秒有效期内，媒体客户端不得用本地默认值
覆盖这些本体字段；超过有效期后允许回退使用媒体客户端状态。
...
`robot.state.status` 仍表示 MQTT 在线状态，因此收到边缘状态时为 `online`；健康异常只在全景设备状态中映射为 `fault`，避免前端误断实时视频。
...
`robot.state` 表示机器人基础状态变化。客户端每次上报 `media/client/status` 后，控制服务先合并管理端设备标识和能力，再由机器人注册服务缓存并广播一条 `robot.state`。后端心跳超时离线扫描也可产生该事件。
...
1. `media/client/status` 只负责媒体客户端存活、视频源清单和客户端实际采集的设备运行态；机器人名称、类型、电量、控制模式、任务、导航、急停等本体业务状态由管理端登记数据及 `eiop/v1/edge/{serialNumber}/status` 提供，`stateSeq` 由控制服务在消费边缘状态时递增。

...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---:|---|
| `deviceId` | string | 是 | 客户端设备 ID。若与管理端平台 ID 不同，应同时在 `status.driverDeviceId` 写入驱动侧 ID。 |
| `bindingId` | string | 是 | 机器人与设备绑定关系 ID。 |
| `scope` | string | 是 | 设备范围，常见 `BODY`、`PAYLOAD`、`SENSOR`、`AUDIO`、`SAFETY`。 |
| `deviceType` | string | 是 | 设备类型，如 `WHEELED_BASE`、`QUADRUPED_BASE`、`DUAL_LIGHT_PTZ`、`MULTI_FUNCTION_BROADCASTER`。 |
...
| `actions` | array[string] | 否 | 客户端动作提示；对外动作能力以管理端登记数据为主，已联调设备可使用控制服务生产兼容动作。 |

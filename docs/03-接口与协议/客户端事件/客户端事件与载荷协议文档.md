...
- `control-service/src/main/java/com/robot/control/messaging/EdgeTaskEventHandler.java`
...
| 边缘任务进度 | MQTT QoS 0 | `eiop/v1/edge/{serialNumber}/tasks/progress` | 任务节点运行、完成、失败或位置变化 | `RobotMediaStatusSubscriber.edgeTaskProgressListener` -> `EdgeTaskEventHandler` -> `/ws/control` 的 `panorama.task.changed` |
| 边缘任务控制结果 | MQTT QoS 0 | `eiop/v1/edge/{serialNumber}/tasks/control-results` | 暂停、恢复、终止命令执行成功或失败 | `RobotMediaStatusSubscriber.edgeTaskControlResultListener` -> `EdgeTaskEventHandler` -> `/ws/control` 的 `panorama.task.changed` |
...
| `/ws/control`、`/ws/media`、`/ws/bigscreen` | Bigscreen BFF -> robot-ui | 大屏 BFF 把 Control/Media 消息转换或桥接给浏览器。 |
...
|---|---|---:|---|
| `event` | string | 是 | 事件名。 |
| `timestamp` | datetime/string | 是 | 后端发出事件的时间，建议统一使用 ISO-8601。 |
...
BFF WebSocket 会把 Control/Media 消息转换或桥接给 robot-ui，事件结构如下：

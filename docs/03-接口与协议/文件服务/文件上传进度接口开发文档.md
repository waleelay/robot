...
> 状态：第一阶段 Media 进度事实层、第二阶段 Management 聚合与双前端按需展示已实现；第三阶段实时失效通知待实施
> 版本：2.0
> 代码基线：2026-09-20
...
当前代码已经实现第 6、10、11、12 章中属于 Media 的核心能力，以及第 7 章 Management 聚合接口、Bigscreen BFF 透明代理和管理端/大屏按列表加载、详情打开、人工刷新触发的进度展示。Control 失效通知、前端事件驱动刷新和机器人 fileId 延迟绑定增强仍属于第三阶段。
...
该接口不再配置独立服务 Token，只允许 Management 通过受控内网访问；部署时禁止将 `/internal/media/**` 映射到公网网关或 Nginx 对外路由。
...
部署时需要在 MinIO 注册 Webhook target，并为 `robot-media` bucket 的 `s3:ObjectCreated:Put` 事件绑定该 target。`auth_token` 配置完整的 `Bearer <token>`，并设置持久化 `queue_dir`；Media 环境变量 `MEDIA_FILE_PROGRESS_WEBHOOK_TOKEN` 只配置其中的 token 值。事件规则可按 `files/` 前缀收缩，Media 仍会再次严格校验 `.upload-parts/{storageUploadId}/part-{number}`。

配置完成后必须用 `mc event target list` 和 `mc event ls` 核对 target 与 bucket 规则，并上传一个测试分片验证 Webhook、Redis Hash 和批量查询响应一致。
...
- 首次创建前按 `robotId + sourceFileId` 锁定 `media_file_source_lock` 行，避免两个并发请求同时判断为不存在。
- `media_file(robot_id, source_file_id)` 唯一约束作为绕过应用锁时的最终一致性保护。
...
- `complete` 使用上传会话数据库行锁串行化同一 `uploadId` 的并发请求。
- 执行者将 `ACTIVE` 更新为 `COMPLETING`，写入执行者和租约后提交短事务，再执行 `listParts` 和 Compose。
- 合并期间周期续租；其他请求命中未过期租约时只返回当前文件状态，不重复 Compose。
- 重试时先检查最终对象；对象存在且大小正确时直接把数据库收口为 `COMPLETED`。
- 状态为 `COMPLETING` 且租约过期时允许新执行者接管；最终对象不存在时重新校验临时分片并继续 Compose。
...
- Media 内部批量接口只允许 Management 通过受控内网访问，不配置额外 Token，且不得映射到公网网关。
...
| `MEDIA_FILE_COMPLETION_LEASE_SECONDS` | `300` | multipart 合并执行权租约，执行期间自动续租 |
...
| 配置 | 建议默认值 | 说明 |
|---|---:|---|
| `EIOP_MEDIA_SERVICE_HOST/PORT` | 环境配置 | Media 内网主机与端口 |
| `EIOP_MEDIA_SERVICE_PROGRESS_PATH` | `/internal/media/files/upload-progress-queries` | Media 批量进度内部路径 |
| `EIOP_MEDIA_SERVICE_PROGRESS_CONNECT_TIMEOUT` | `500ms` | 连接超时 |
| `EIOP_MEDIA_SERVICE_PROGRESS_READ_TIMEOUT` | `2s` | 批量查询超时 |
| 代码约束 | `100` | 对外单次执行记录上限 |
| 代码约束 | `500` | 单次下游文件上限 |
...
1. 已实现 MinIO Webhook、Redis Lua 投影、内部批量查询和缓存缺失回源。
2. 已实现动态分片，限制分片总数不超过 10,000。
3. 已增加文件/上传会话唯一约束、`COMPLETING` 完成保护和 Compose 后恢复。
4. 待第三阶段实现向 EIOP Control 发布最小失效事件。
5. 后续联调环境继续补充真实 Redis、MinIO 重复事件及故障恢复集成测试。

### 17.2 EIOP Management

1. 已在现有 Media port 增加批量进度能力，由独立短超时基础设施适配器调用 Media。
2. 已新增执行记录产物进度应用服务、DTO 和公开 Controller。
3. 已批量加载执行记录与产物关系，复用 `task.record.view` 和数据范围校验后聚合。
4. 已实现 Media 故障返回 `available=false`，不影响执行记录列表。
5. 已覆盖权限、待绑定、字节加权和下游降级测试。
...
1. 已验证现有执行记录通配代理可转发新增 POST 子路径。
2. 已增加代理路径与 POST 请求回归测试；请求体、认证头和 Management 响应继续由通用代理透明转发。
...
1. 已在 API 层增加批量产物进度请求。
2. 已在列表增加汇总列，在视频弹窗和详情增加逐文件状态；仅 `READY` 文件进入播放。
3. 当前仅在列表查询、详情打开和人工刷新时读取，不使用固定轮询。
4. 第三阶段再接入 `source=media`、`task.changed.v1 + EXECUTION`、防抖和 single-flight。
5. 第三阶段再实现产物首次进入 `READY` 时事件驱动刷新 replay。
...
1. 已在 `new-bi.js` 增加 BFF 批量进度接口。
2. 已在列表和详情复用与管理端一致的状态文案和聚合含义。
3. 当前仅在列表查询、详情打开和人工刷新时读取，不使用固定轮询。
4. 已限制未就绪视频不请求播放地址；第三阶段再监听 `taskRefreshRevision` 并刷新当前页进度。

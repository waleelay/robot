...
| 字段 | 环境变量 | 说明 |
|---|---|---|
| `robot_id` | `ROBOT_ID` | 机器人 ID，默认 `test001` |
...
默认摄像头 ID：`test001` 为 `camera01/camera02/camera03`，`test002` 为 `camera04/camera05/camera06`。实际选流优先按 `deviceId/cameraId` 找摄像头配置，再根据 `quality` 使用 main 或 sub RTSP。

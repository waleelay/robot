<template>
  <div class="redDetection" ref="bottomRight">
    <div
      id="divPlugin4"
      class="plugin1"
...
export default {
  ...
  methods: {
    init() {
      WebVideoCtrl.I_InitPlugin({
        ...
        iWndowType: 1,
        cbSelWnd: (xmlDoc)=> {
          g_iWndIndex4 = parseInt($(xmlDoc).find('SelectWnd').eq(0).text(), 10)
        },
        ...
        cbInitPluginComplete: ()=> {
          WebVideoCtrl.I_InsertOBJECTPlugin('divPlugin4').then(() => {
            // 检查插件是否最新
    ...
    //开始播放
    startRealPlay() {
      var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex4)
      var startRealPlay = () => {
        WebVideoCtrl.I_StartRealPlay(`${this.ip}_${this.port}`, {
          iStreamType: 1,
          iChannelID: 2,
          bZeroChannel: false,
          iWndIndex: g_iWndIndex4, // 明确指定窗口索引
          success: () =>{
            console.log('开始预览成功！')
          },
  ...
  beforeDestroy() {
    ...
    WebVideoCtrl.I_Stop({
      iWndIndex: g_iWndIndex4,
      success: () => {
        WebVideoCtrl.I_RemoveOBJECTPlugin("divPlugin4");
        console.log("销毁成功");
      },
    });
...
.redDetection {
  ...
  &::before {
    ...
    height: 232px;
    position: absolute;
    background-image: url("../../assets/images/largescreen/demo-img1.png");

<template>
  <div class="deviceTalk"
    ...
    <!--    机器人控制-->
    <div class="item-box item-box-new">
      <div id="divPlugin3"
           class="plugin1"
           style="height: 0px;width: 0px;"></div>
      <!-- <div class="title">机器犬控制</div> -->
      <div class="item-inner-box">
        <div class="control-box custom">
          <div class="control-item"
               :class="{ 'is-disabled': deviceData.onDockState === '1' }"
               :title="item.label"
               v-for="(item, index) in controlList"
               :key="index"
               @mousedown="handleControlRotation(item)"></div>
          <div class="label">移动</div>
        </div>
        <div class="btn-box flx-justify-between flex-column wp354">
          <el-button type="primary"
                     class="w100"
                     @click="controlDevice('startup')">一键开机</el-button>
          <div class="flx-justify-between mt20 w100">
            <el-button :disabled="deviceData.onDockState === '1'"
                       type="primary"
                       class="wp167"
                       @click="handleControl(btnObj['zuoyi'].value)">左移</el-button>
            <el-button :disabled="deviceData.onDockState === '1'"
                       type="primary"
                       class="wp167 ml20"
                       @click="handleControl(btnObj['youyi'].value)">右移</el-button>
          </div>
          <div class="flx-justify-between mt20 w100">
            <el-button :disabled="deviceData.onDockState === '1'"
                       type="primary"
                       class="wp100"
                       @click="handleControl(btnObj['zhanli'].value)">站立</el-button>
            <el-button :disabled="deviceData.onDockState === '1'"
                       type="primary"
                       class="wp100"
                       @click="handleControl(btnObj['paxia'].value)">趴下</el-button>
            <el-button :disabled="deviceData.onDockState === '1'"
                       type="primary"
                       class="wp100"
                       @click="handleControl(btnObj['tztb'].value)">停止踏步</el-button>
          </div>
          <div class="flx-justify-between mt20 w100">
            ...
            <el-button :disabled="deviceData.onDockState === '1'"
                       type="primary"
                       class="wp100"
                       @click="handleControl(btnObj['jiting'].value)">急停</el-button>
            <el-select :disabled="deviceData.onDockState === '1'"
                       v-model="butaiValue"
                       placeholder="切换步态"
                       @change="handleChangebutai"
                       class="qhbt-select wp100 ml10"
                       title="切换步态">
              <el-option label="行走"
                         :value="0" />
              <el-option label="普通楼梯"
                         :value="1" />
              <el-option label="斜坡/防滑"
                         :value="2" />
              <el-option label="感知楼梯"
                         :value="4" />
            </el-select>
            <el-button :disabled="deviceData.onDockState === '1'"
                       type="primary"
                       class="wp100"
                       @click="setSpeed">设定速度</el-button>
          </div>
          <div class="flx-justify-between mt20 w100">
            <el-button type="primary"
                       plain
                       class="wp167"
                       @click="controlDevice('shutdown')">一键关机</el-button>
            <el-button type="primary"
                       plain
                       class="wp167"
                       @click="exit()">退出控制</el-button>
          </div>
        </div>
      </div>
    </div>
    ...
    <SpeedDialog @changeSpeed="changeSpeed"
                 ref="speedRef" />
  </div>
</template>
...
import SpeedDialog from './speed.vue'
...
export default {
  ...
  data() {
    return {
      ...
      btnObj: {
        ...
        sdsd: { label: "设定速度", value: 0 },
        close: { label: "一键关机", value: 0 },
        exit: { label: "退出控制", value: 0 },
      },
      controlList2: [
        {
          label: "上",
  ...
  methods: {
    ...
    getMotionControlApi(command, commandValue) {
      this.motion = command
      motionControl(command, commandValue).then(res => {
    ...
    async yuntaiControl(dwPTZCommand, dwStop) {
      try {
        const response = await axios.get(`${location.origin}/yuntai`, {
          params: {
            dwPTZCommand: dwPTZCommand,
            // dwStop: dwStop,
          },
        })
      } catch (err) { }

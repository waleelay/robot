export default {
  ...
  methods: {
    ...
    initWebSocket() {
      ...
      // 连接成功：自动启动当前设备心跳
      this.ws.onopen = () => {
        this.addLog('dogControlWebSocket连接成功')
      ...
      this.ws.onmessage = (e) => {
        ...
        const dogId = Number(this.dogId);
        if (data.type === "status" && deviceId === dogId) {
      ...
      if (this.ws.readyState === WebSocket.OPEN) {
        ...
        this.heartbeatTimerMap[deviceId] = setInterval(() => {
          ...
          if (this.deviceStatus === 'offline' && this.ws.readyState === WebSocket.OPEN) {
            this.ws.send(JSON.stringify({ type: 'startHeartbeat', deviceId }))
            this.addLog(`【心跳检测】设备${deviceId}离线${this.deviceStatus}，重新启动`)
          }
        }, 2000)
     ...
     */
    handleDeviceChange() {
      this.clearAllHeartbeatTimer() // 停止所有旧设备心跳
      if (this.dogId) {
        this.addLog(`选中设备：${this.dogId}`)
        // 查询状态 + 启动新设备心跳
        if (this.ws.readyState === WebSocket.OPEN) {
          this.ws.send(JSON.stringify({ type: 'queryStatus', deviceId: this.dogId }))
          this.startDeviceHeartbeat(this.dogId)
        }
      }
    },

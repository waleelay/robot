...
export default {
  ...
  methods: {
    ...
    connectMqtt: function() {
      ...
      let mqttclient = mqtt.connect(`ws://192.168.124.94:8083/mqtt`);
        ...
        if (msg.msg_type == "result") {
          ...
          for (let i = 0; i < 4; i++) {          
            if (this.ZQL_videosInfos[i]) {
              if (id == this.ZQL_playingSource[i] && msg.data.alg.name == this.ZQL_videosInfos[i].alg) {                
                this.setAlarms(msg.data.reserved_data, i);
                if (this.ZQL_videosInfos[i] && this.ZQL_videosInfos[i].canvasTimeout) {
    ...
    // 请求后端接口
    getSources() {
      ZQLGLOBAL.serverIp = 'http://192.168.124.94'
      // draw_size: [1280,720]
    ...
    sysArgs() {
      const url = process.env.NODE_ENV === 'development' ? 'http://192.168.124.94:9091/ks/source' : `${ZQLGLOBAL.tunnel}`
      getTunnels(url).then(tunnelRes => {
        if (tunnelRes.data.enable == false) {
          getSysArgs(`${ZQLGLOBAL.sysArgs}`).then(res => {

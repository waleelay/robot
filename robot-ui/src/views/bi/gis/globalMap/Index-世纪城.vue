...
export default {
  ...
  data () {
    return {
      ...
      waypoints: [
        ...
        {lat: 30.5615111, lng: 104.0736232, desc: '起始地'},
        {lat: 30.5610687, lng: 104.0745834},
        {lat: 30.5598084, lng: 104.0735765},
        {lat: 30.5600497, lng: 104.0730264, desc: '目的地'}
      ],
      pathLayers: [],
      movementPath: null,
      isMoving: false,
  ...
  methods: {
    ...
    initMap(){
      ...
      // 地图底图
      L.tileLayer('/tdt/tiles/new/shijicheng/{z}/{x}/{y}.png', {
    ...
    initPoints() {
      ...
      // ]
      const latLngs = [
        { lat: 30.5615111, lng: 104.0736232, status1: 1 },
        { lat: 30.5610687, lng: 104.0745834, status1: 2 },
        { lat: 30.5598084, lng: 104.0735765, status1: 0 },
        { lat: 30.5600497, lng: 104.0730264, status1: 3 },
      ]
    ...
    showDashedArea(flag) {
      ...
      const polygonPoints = [
        ...
        [30.5615111,104.0736232],
        [30.5610687,104.0745834],
        [30.5598084,104.0735765],
        [30.5600497,104.0730264]
      ];
      // 如果已有虚线多边形，移除
    ...
    // 测试demo
    // ---- 工具函数：计算两点经纬度之间的距离（米）使用 Haversine 公式 ----
    haversineDistance() {
      const { lat: lat1, lng: lng1 } = this.waypoints[1]
      const { lat: lat2, lng: lng2 } = this.waypoints[2]
    ...
    updateMarker(step) {
      if (!step) {
        // this.movingMarker = L.circleMarker([this.waypoints[1].lat, this.waypoints[1].lng], {
        //   radius: 8,
        //   color: '#e74c3c',
        //   fillColor: '#e74c3c',
        //   fillOpacity: 0.8,
        //   weight: 2
        // }).addTo(this.map);
        const points = L.latLng(this.waypoints[1].lat || 39.54, this.waypoints[1].lng || 116.23)        
        this.movingMarker = L.marker(points, { 
            icon: this.getIcon({ ...this.robotList[1], points }),
            zIndexOffset: 1000,
            // riseOnHover: true
          }).addTo(this.map)
      }
      const totalDistance = this.haversineDistance();
      const t = Math.min(step / Math.ceil(totalDistance / 0.2), 1);
      const pos = this.interpolate(t);
      this.movingMarker.setLatLng([pos.lat, pos.lng]);

      // 更新信息
      const moved = t * totalDistance;
    ...
    startAnimation() {
      // 0.2m每单位移动一次
      const totalSteps = Math.ceil(this.haversineDistance() / 0.2);
      this.testTimer = setInterval(() => {
          this.currentStep++;
          this.updateMarker(this.currentStep);
      }, 200); // 每50毫秒走一步，相当于每秒20步，即每秒移动 20*0.2=4米/秒，速度可调
    },
    stopAnimation() {
      if (this.testTimer) {

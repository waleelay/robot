...
export default {
  ...
  async mounted(){
    ...
    this.initMap();
    // 设置地图旋转 -45度
    if (this.map && this.map.setBearing) {
  ...
  methods: {
    ...
    initMap(){
      ...
      // 监听 bearing 变化，重新绘制当前矩形（如果存在）
      this.map.on('rotate', () => {
        if (this.currentPolygon && this.activeMarkerIndex) {
          // 重新绘制当前激活的矩形，实现矩形视觉上不跟随旋转
          const activeMarker = this.getMarkerById(this.activeMarkerIndex);
          if (activeMarker) {
              this.drawRectangleAroundMarker(activeMarker);
          }
        }
      });
      this.map.on('unload', () => {
    ...
    getMarkerById(markerIndex) {
      let found = null;
      this.markersLayer.eachLayer(layer => {
        if (layer instanceof L.Marker && layer.meta && layer.meta.index === markerIndex) {
            found = layer;
        }
      });
      return found;
    ...
    initPoints() {
      ...
      this.dogList.map((item, index) => {
        ...
        const activeIcon = L.divIcon({
            ...
            html: `<div class="marker-pin-active"><span>⭐</span></div>`,
            iconSize: [38, 38],
            // popupAnchor: [0, -19]
        });
        L.marker(item.points, { icon: activeIcon }).addTo(this.markersLayer)
        // 存储扩展数据，方便后续使用
    ...
    updatePopups(index) {
      ...
      if (this.pointMarkers[index]) {
        ...
        if (this.pointMarkers[index].getPopup()) {
          ...
          this.setupHoverWithDebounce(index, 300)
        }
      }
    },

    // 添加带防抖的鼠标悬停事件
    setupHoverWithDebounce(markerIndex, delay = 200) {
      const marker = this.pointMarkers[markerIndex]
      let hoverTimer;
    ...
    drawRectangleAroundMarker(marker) {
      ...
      if (!meta) return; 
      // 计算矩形边界 bounds
      // console.log(meta.points);
      // marker.getLatLng()
      const rotatedCorners = this.getRotatedCompensatedPolygon(meta.points);
      
      // 清除之前的高亮和矩形图层
      this.clearSelectionAndHighlight();
      
      // 1. 绘制矩形 (使用 Leaflet 的 Rectangle 继承自Polygon，支持样式)
      console.log(11, rotatedCorners);
      ...
      // 计算标签位置：取旋转后矩形的北边界中点（根据实际多边形顶点计算最高点）
      let maxLat = -Infinity;
      let centerLngSum = 0;
      rotatedCorners.forEach(corner => {
          if (corner.lat > maxLat) maxLat = corner.lat;
          centerLngSum += corner.lng;
      });
      const labelLng = centerLngSum / rotatedCorners.length;
      let labelPos = L.latLng(maxLat, labelLng);
      // 向上微调偏移（转换为屏幕偏移再转回地理，简单偏移纬度）
      const pixelUp = this.map.latLngToLayerPoint(labelPos);
      const adjustedPixel = L.point(pixelUp.x, pixelUp.y - 25);
      labelPos = this.map.layerPointToLatLng(adjustedPixel);
      
      const labelIcon = L.divIcon({
        className: 'region-label-div',
        html: `<div class="region-label" style="transform: rotate(0deg); color: #fff">监区宿舍AAA</div>`,
        iconSize: [null, null],
        iconAnchor: [41, 115],
        popupAnchor: [0, -10]
      });
      const labelMarker = L.marker(meta.points, {
          icon: labelIcon,
          // 不可交互，防止点击矩形
          interactive: false,
          zIndexOffset: 1000
      ...
      this.activeMarkerIndex = meta.index;
    },
    getRotatedCompensatedPolygon(centerLatLng) {
      const bearingDeg = this.map.getBearing();
      const lat = centerLatLng.lat;
      const lng = centerLatLng.lng;
      const widthMeters = 55; // 东西宽度 (米)
      const heightMeters = 90; // 南北高度 (米)
      if (!this.map || bearingDeg === 0) {
        const corners = this.getRectangleCorners(centerLatLng, widthMeters, heightMeters);
        return corners;
      }
      // 获取矩形原始四个角（地理正北矩形）
      const originalCorners = this.getRectangleCorners(centerLatLng, widthMeters, heightMeters);
      // 获取地图中心点的像素坐标作为旋转中心
      const centerPixel = this.map.latLngToLayerPoint(centerLatLng);
      const rad = -bearingDeg * Math.PI / 180; // 反向旋转，抵消地图旋转
      
      const rotatedCorners = originalCorners.map(corner => {
          const pixel = this.map.latLngToLayerPoint(corner);
          // 相对于中心偏移
          ...
          const newPixel = L.point(centerPixel.x + newX, centerPixel.y + newY);
          return this.map.layerPointToLatLng(newPixel);
      });
      return rotatedCorners;
    },
    getRectangleCorners(centerLatLng, widthMeters, heightMeters) {
      const lat = centerLatLng.lat;
      const lng = centerLatLng.lng;
      ...
      const nw = L.latLng(lat + deltaLat, lng - deltaLng);
      const ne = L.latLng(lat + deltaLat, lng + deltaLng);
      const se = L.latLng(lat - deltaLat, lng + deltaLng);
      const sw = L.latLng(lat - deltaLat, lng - deltaLng);
      return [nw, ne, se, sw];
    },

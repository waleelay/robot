...
test('来电终态登记有界保留并阻止迟到 accepted 重新激活', () => {
  ...
  assert.equal(registry.markAll(['call-2', '', 'call-3'], 'page-leave'), 2)
  assert.equal(registry.reason('call-2'), 'page-leave')
  assert.equal(registry.reason('call-3'), 'page-leave')
  for (const handler of [...timers.values()]) handler()
  assert.equal(registry.has('call-1'), false)
  assert.equal(registry.has('call-2'), false)
  assert.equal(registry.has('call-3'), false)
})
...
test('普通来电与现场来电都以统一终态登记拦截乱序 accepted', () => {
  ...
  assert.match(fieldCallSource, /fieldCallTerminals\.has\(callId\)/)
})

test('普通对讲和现场呼叫共享唯一浏览器通话租约', () => {
  const coordinator = new coordinatorModule.MediaCallCoordinator()
  const regular = coordinator.acquire('robot:camera-1', 'robot-intercom', 'talk:1')
  assert.ok(regular)
  assert.equal(coordinator.ownedBy(regular, 'talk:1'), true)
  assert.equal(coordinator.ownedBy(regular, 'talk:2'), false)
...
test('对讲入口只收口自己启动的通话，地图弹窗关闭会等待释放其拥有的通话', () => {
  assert.doesNotMatch(mapPopupSource, /stopIntercomLifecycle/)
  assert.match(mapPopupSource, /await talk\.releaseOwnedIntercom\(\)/)
  assert.match(robotControlPopupSource, /<Talk[\s\S]*?ref="talkRef"[\s\S]*?:target-robot-id="effectiveRobotId"[\s\S]*?:target-robot="controlRobot"/)
  assert.match(robotCarControlPopupSource, /<Talk[\s\S]*?ref="talkRef"[\s\S]*?:target-robot-id="effectiveRobotId"[\s\S]*?:target-robot="controlRobot"/)
  assert.match(robotControlPopupSource, /class="close mr10 ml10" @click="show\(false\)"/)
  assert.match(robotCarControlPopupSource, /class="close mr10 ml10" @click="show\(false\)"/)
  assert.match(talkSource, /intercomOwnerId: `talk:\$\{\+\+talkOwnerSequence\}`/)
  assert.match(talkSource, /return this\.targetRobotId \|\| this\.\$store\.getters\['websocketRobot\/getSelectedRobotId'\] \|\| ''/)
  assert.match(talkSource, /String\(this\.targetRobot\?\.robotId \|\| ''\) === String\(this\.targetRobotId\)/)
  assert.match(talkSource, /talkAvailable\(\) \{[\s\S]*this\.selectedRobotId && this\.selectCamera\.key && this\.selectedRobot\.status === 'online'/)
  assert.match(talkSource, /async handleTalk\(\) \{[\s\S]*if \(!this\.talkAvailable\) return/)
  assert.match(talkSource, /ownerId: this\.intercomOwnerId/)
  assert.match(talkSource, /return await this\.stopOwnedIntercom\(\{ cameraKey, ownerId: this\.intercomOwnerId \}\)/)
  assert.match(talkSource, /beforeDestroy\(\) \{[\s\S]*this\.releaseOwnedIntercom\(\)/)
  assert.match(websocketSource, /async stopOwnedIntercom[\s\S]*requestTimeoutMs: MEDIA_CLEANUP_STOP_TIMEOUT_MS/)
  assert.match(websocketSource, /cameraOrKey: current \|\| key/)
})
...
test('普通对讲把取消信号传入 Room 连接，挂断确认失败仍执行本地关闭', () => {
  ...
  const start = websocketSource.indexOf('async performHangupIntercom')
...
test('入口销毁与页面退出并发挂断时复用同一个收口操作', () => {
  assert.match(websocketSource, /const intercomStopOperations = new Map\(\)/)
  const start = websocketSource.indexOf('hangupIntercom({ dispatch }, payload)')
  const end = websocketSource.indexOf('async performHangupIntercom', start)
  const block = websocketSource.slice(start, end)
  assert.match(block, /runOnce\(intercomStopOperations, key/)
  assert.match(block, /dispatch\('performHangupIntercom', options\)/)
})
...
test('页面退出与注销清理边界分离', () => {
  ...
  assert.match(pageBlock, /reason: '页面退出'/)
  assert.match(pageBlock, /Promise\.allSettled/)
  assert.match(pageBlock, /\.\.\.intercomStartOperations\.keys\(\)/)
  assert.match(pageBlock, /cameraOrKey: current/)
...
test('页面退出先终态化待接听呼叫，迟到 accepted 只做服务端收口', () => {
  const start = websocketSource.indexOf('async terminateIncomingCalls')
  const end = websocketSource.indexOf('async stopPageMediaSessions', start)
  const block = websocketSource.slice(start, end)
  assert.match(block, /pendingIncomingCallLeases\.keys\(\)/)
  assert.match(block, /pendingIncomingIntercomKeys\.keys\(\)/)
  assert.match(block, /incomingCallTerminals\.markAll\(callIds, reason\)/)
  assert.match(block, /action: 'reject', callId, silent: true/)
  assert.match(block, /await dispatch\('hangupIncomingCall', \{ requestTimeoutMs, silent \}\)/)
  assert.match(block, /commit\('SET_ACTIVE_INCOMING_CALL', null\)/)
  assert.match(websocketSource, /if \(incomingCallTerminals\.has\(callId\)\) \{[\s\S]*stopIntercom\(event\.payload\.intercom\.sessionId\)/)
})

test('注销时并行收口媒体会话且每路请求有超时上限', () => {
  const start = websocketSource.indexOf('async stopAllCameraSessions')
  const end = websocketSource.indexOf('async terminateIncomingCalls', start)
  const block = websocketSource.slice(start, end)
  assert.match(block, /Promise\.allSettled/)
  assert.match(block, /stopTimeoutMs: MEDIA_CLEANUP_STOP_TIMEOUT_MS/)
  assert.match(block, /requestTimeoutMs: MEDIA_CLEANUP_STOP_TIMEOUT_MS/)
  assert.match(block, /silent: true/)
  assert.match(websocketSource, /const MEDIA_CLEANUP_STOP_TIMEOUT_MS = 5000/)
  assert.match(mediaApiSource, /stopVideoSession\(sessionId, \{ timeout \} = \{\}\)/)
  assert.match(mediaApiSource, /\.\.\.\(timeout \? \{ timeout \} : \{\}\)/)
  assert.match(mediaApiSource, /stopIntercom\(sessionId, \{ timeout \} = \{\}\)/)
  assert.match(mediaApiSource, /stopLiveRecording\(sessionId, fileId, \{ timeout \} = \{\}\)/)
  assert.match(websocketSource, /stopLiveRecording\(camera\.session\.sessionId, camera\.activeRecording\.fileId, \{[\s\S]*timeout: requestTimeoutMs/)
})
...
test('现场呼叫退出会清理活动通话之外的所有残留 session', () => {
  ...
  assert.match(block, /commit\('SET_CONNECTED', false\)/)
  assert.match(block, /fieldCallTerminals\.markAll\(pendingCallIds, 'page-leave'\)/)
  assert.match(block, /silent: true/)
  assert.doesNotMatch(block, /fieldCallTerminals\.clear\(\)/)
  assert.match(fieldCallSource, /if \(!state\.connected \|\| fieldCallTerminals\.has\(callId\)\)/)
})

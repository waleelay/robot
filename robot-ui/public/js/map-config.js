var gisConfig = {
  area: {
    key: 'qihang'
  },
  /**
   * MapTool「点位」按钮禁用策略
   * true：没有充电点/巡检点/门禁点时禁用
   * false：没有点位数据时禁用（地图展示仍只渲染上述三类点）
   */
  disablePointWithoutCharge: true,
  nanchong: {
    key: 'nanchong',
    center: [30.7478613352993, 106.03655278081857],
    zoom: [12, 18],
    // 18级转角45度
    rotate: 45,
    polygonPoints: [
      [ 30.748881699556307, 106.03529721123913 ],
      [ 30.7478613352993, 106.03655278081857 ],
      [ 30.747140651711415, 106.03579008416635 ],
      [ 30.74812517003875, 106.03462176616857 ],
    ],
    wayPoints: [
      { lat: 30.74858822373491, lng: 106.03506952591954, desc: '起始地' },
      { lat: 30.748379525423942, lng: 106.03487979487186 },
      { lat: 30.748229520051265, lng: 106.03472042553155 },
      { lat: 30.74812517003875, lng: 106.03462176616857 },
      { lat: 30.747603565260476, lng: 106.03519074431382 },
      { lat: 30.747271049335637, lng: 106.03561559199737 },
      { lat: 30.747140651711415, lng: 106.03579008416635 },
      { lat: 30.7478613352993, lng: 106.03655278081857 },
      { lat: 30.7483112149064, lng: 106.03603690298392 },
      { lat: 30.748686106849632, lng: 106.0355627433318 },
      { lat: 30.748881699556307, lng: 106.03529721123913, desc: '目的地' }
    ]
  },
  qihang: {
    key: 'qihang',
    center: [30.5555839,104.0606658],
    zoom: [12, 16],
    // 18级不旋转
    rotate: 0,
    polygonPoints:[
      [30.5567849,104.0603011],
      [30.5567803,104.0615349],
      [30.5555839,104.0615993],
      [30.5555469,104.0603172]
    ],
    wayPoints: [
      { lat: 30.5567849, lng: 104.0603011, desc: '起始地' },
      { lat: 30.5567803, lng: 104.0615349 },
      { lat: 30.5555839, lng: 104.0615993 },
      { lat: 30.5555839, lng: 104.0606658 },
      { lat: 30.5555469, lng: 104.0603172, desc: '目的地' }
    ]
  },
  yunnan: {
    key: 'yunnan',
    center: [24.3357337,102.5560676],
    zoom: [12, 18],
    // 18级不旋转
    rotate: 0,
    polygonPoints:[
      [24.3345387,102.5541591],
      [24.3335859,102.5543160],
      [24.3357337,102.5567648],
      [24.3336097,102.5567996]
    ],
    wayPoints: [
      { lat: 24.3345387, lng: 102.5541591, desc: '起始地' },
      { lat: 24.3357337, lng: 102.5560676 },
      { lat: 24.3350111, lng: 102.5545513 },
      { lat: 24.3357337, lng: 102.5567648 },
      { lat: 24.3336097, lng: 102.5567996, desc: '目的地' }
    ]
  }
}

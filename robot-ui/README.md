...
SLAM 地图切换时，点位和固定摄像头作为首帧资源优先写入，任务路径在请求完成后补齐，不阻塞地图展示。
Canvas 位图按当前缩放比例和设备像素比创建；障碍网格仅在开启安全区域校验时构建，避免大图缩小展示时
仍按原始分辨率重复分配和扫描像素。大图逐像素调色在 Web Worker 的 OffscreenCanvas 中完成，主线程只在
蓝色成品就绪后一次性换帧；不支持离屏处理时保持加载态并同步生成成品，不展示白色中间帧。

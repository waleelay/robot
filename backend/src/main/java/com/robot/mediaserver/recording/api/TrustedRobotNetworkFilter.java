...
public class TrustedRobotNetworkFilter extends OncePerRequestFilter {
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) {
        ...
        boolean robotRecordingPath = path.startsWith("/api/media/recording-uploads")
                || (path.startsWith("/api/media/recordings/") && path.endsWith("/status"));
        return !robotRecordingPath || !properties.getRecording().isTrustedRobotNetworkEnabled();
    }
    ...
    private boolean trusted(String address) {
        return Arrays.stream(properties.getRecording().getTrustedRobotCidrs().split(","))
                .map(String::trim)

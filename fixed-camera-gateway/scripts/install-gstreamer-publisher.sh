...
patch_livekit_ice_tcp="${PATCH_LIVEKIT_ICE_TCP:-true}"
...
if [ -n "$src" ]; then
  ...
  git -c "http.version=${git_http_version}" clone --depth 1 "$repo" "$tmp" || {
    ...
    echo "retry with:"
    echo "GIT_HTTP_VERSION=HTTP/1.1 sh fixed-camera-gateway/scripts/install-gstreamer-publisher.sh"
    ...
    echo "GSTREAMER_PUBLISHER_SRC=/path/to/gstreamer-publisher sh fixed-camera-gateway/scripts/install-gstreamer-publisher.sh"
    exit 1
  }
  build_dir="$tmp"
fi
...
if [ "$patch_livekit_ice_tcp" = "true" ]; then
  (cd "$build_dir" && GOPROXY="$go_proxy" go mod download github.com/livekit/server-sdk-go/v2) || {
    echo "failed to download livekit server-sdk-go before patching." >&2
    exit 1
  }
  livekit_sdk_dir="$(cd "$build_dir" && GOPROXY="$go_proxy" go list -m -f '{{.Dir}}' github.com/livekit/server-sdk-go/v2)"
  livekit_transport_file="${livekit_sdk_dir}/transport.go"
  [ -f "$livekit_transport_file" ] || {
    echo "livekit server-sdk-go transport.go not found: $livekit_transport_file" >&2
    exit 1
  }
  chmod -R u+w "$livekit_sdk_dir"

  if ! grep -q 'fixed-camera-gateway: livekit ice tcp fallback' "$livekit_transport_file"; then
    # Pion 默认仅收集 UDP 候选。启用主动 ICE-TCP 后，UDP 被现场网络阻断时可复用
    # LiveKit 已开放的 rtc.tcp_port 建立媒体连接。
    livekit_transport_tmp="${livekit_transport_file}.fixed-camera-gateway"
    sed '/se := webrtc.SettingEngine{}/a\
    configureICETransport(\&se)
' "$livekit_transport_file" > "$livekit_transport_tmp"
    mv "$livekit_transport_tmp" "$livekit_transport_file"
    cat >> "$livekit_transport_file" <<'EOF'

// fixed-camera-gateway: livekit ice tcp fallback
func configureICETransport(se *webrtc.SettingEngine) {
...
(cd "$build_dir" && GOPROXY="$go_proxy" go build -o gstreamer-publisher .) || {
  ...
  echo "Retry with: PATCH_GST_DEBUG_MESSAGE_ID=true GOPROXY=https://goproxy.cn,direct sh fixed-camera-gateway/scripts/install-gstreamer-publisher.sh"
  echo "If dependency downloads time out, retry with another GOPROXY, for example:"
  echo "GOPROXY=https://goproxy.cn,direct sh fixed-camera-gateway/scripts/install-gstreamer-publisher.sh"
  echo "GOPROXY=https://proxy.golang.com.cn,direct sh fixed-camera-gateway/scripts/install-gstreamer-publisher.sh"
  echo "GOPROXY=direct sh fixed-camera-gateway/scripts/install-gstreamer-publisher.sh"
  exit 1
}
